...
namespace HWD::Cortex {
    ...
    namespace M4::DWT {
        ...
        class REG_CTRL {
        public:
            REG_CTRL(Probe::IProbe* probe, uint32_t regAddr) : m_Probe(probe), m_Address(regAddr) {
            }
            REG_CTRL(const REG_CTRL& other) = delete; // do not allow copy
            REG_CTRL(REG_CTRL&& other)      = delete; // do not allow move

...
public:
    QualType(const Node *Child_, Qualifiers Quals_) :
        Node(KQualType, Child_->RHSComponentCache, Child_->ArrayCache, Child_->FunctionCache), Quals(Quals_), Child(Child_) {
    }
...
public:
    ReferenceType(const Node *Pointee_, ReferenceKind RK_) : Node(KReferenceType, Pointee_->RHSComponentCache), Pointee(Pointee_), RK(RK_) {
    }
...
public:
    FunctionEncoding(const Node *Ret_,
                     const Node *Name_,
                     NodeArray Params_,
                     const Node *Attrs_,
                     Qualifiers CVQuals_,
...
public:
    SyntheticTemplateParamName(TemplateParamKind Kind_, unsigned Index_) : Node(KSyntheticTemplateParamName), Kind(Kind_), Index(Index_) {
    }
...
public:
    NonTypeTemplateParamDecl(Node *Name_, Node *Type_) : Node(KNonTypeTemplateParamDecl, Cache::Yes), Name(Name_), Type(Type_) {
    }
...
public:
    TemplateTemplateParamDecl(Node *Name_, NodeArray Params_) : Node(KTemplateTemplateParamDecl, Cache::Yes), Name(Name_), Params(Params_) {
    }
        ...
        switch (SSK) {
            ...
            case SpecialSubKind::string:
                S += "std::basic_string<char, std::char_traits<char>, "
                     "std::allocator<char> >";
                break;
            case SpecialSubKind::istream: S += "std::basic_istream<char, std::char_traits<char> >"; break;
            case SpecialSubKind::ostream: S += "std::basic_ostream<char, std::char_traits<char> >"; break;
            case SpecialSubKind::iostream: S += "std::basic_iostream<char, std::char_traits<char> >"; break;
        }
    }
};
...
public:
    MemberExpr(const Node *LHS_, StringView Kind_, const Node *RHS_) : Node(KMemberExpr), LHS(LHS_), Kind(Kind_), RHS(RHS_) {
    }
...
public:
    CastExpr(StringView CastKind_, const Node *To_, const Node *From_) : Node(KCastExpr), CastKind(CastKind_), To(To_), From(From_) {
    }
...
public:
    ConversionExpr(const Node *Type_, NodeArray Expressions_) : Node(KConversionExpr), Type(Type_), Expressions(Expressions_) {
    }
...
struct AbstractManglingParser {
    ...
    public:
        ScopedTemplateParamList(AbstractManglingParser *Parser) : Parser(Parser), OldNumTemplateParamLists(Parser->TemplateParams.size()) {
        ...
        case 'c':
            switch (look(1)) {
                ...
                case 'v': {
                    ...
                    SwapAndRestore<bool> SavePermit(PermitForwardTemplateReferences, PermitForwardTemplateReferences || State != nullptr);
...
Node *AbstractManglingParser<Derived, Alloc>::parseType() {
    ...
    switch (look()) {
        ...
        case 'K': {
            ...
            if (look(AfterQuals) == 'F' || (look(AfterQuals) == 'D' && (look(AfterQuals + 1) == 'o' || look(AfterQuals + 1) == 'O' ||
...
Node *AbstractManglingParser<Derived, Alloc>::parseEncoding() {
    ...
    if (consumeIf('v'))
        return make<FunctionEncoding>(ReturnType, Name, NodeArray(), Attrs, NameInfo.CVQualifiers, NameInfo.ReferenceQualifier);

...
namespace L0::UI {
    ...
    AboutWindow::AboutWindow(QWidget* parent) : QWidget(parent), ui(std::make_unique<Ui::AboutWindow>()) {
        ui->setupUi(this);
        setWindowTitle("About " + QStringLiteral(CFXS_PROGRAM_NAME));
        setFixedSize(size());
        ...
        ui->label_Version->setText(QStringLiteral(CFXS_PROGRAM_NAME) + QStringLiteral(" ") + QStringLiteral(CFXS_VERSION_STRING) +
                                   QStringLiteral("\n") + buildTime);
    }
